...
const KaikkiTulokset = () => {
  ...
  if (KayttajaContext) {
    ...
    <React.Fragment>
        {KayttajaContext.kayttajantiedot ? KayttajaContext.kayttajantiedot.map((kayttajanTieto) => (
           <TulosTiedot key= {kayttajanTieto&&kayttajanTieto.id} kayttajanTieto={kayttajanTieto} /> 
        ))
        : null}
        </React.Fragment>
